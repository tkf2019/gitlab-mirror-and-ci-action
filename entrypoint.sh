...
sh -c "git config --global credential.username $GITLAB_PROJECT_NAME"
...
pipeline_id=$(curl --header "PRIVATE-TOKEN: $GITLAB_PROJECT_TOKEN" --silent "https://${GITLAB_HOSTNAME}/api/v4/projects/${GITLAB_PROJECT_ID}/repository/commits/${branch_uri}" | jq '.last_pipeline.id')
...
do
   sleep $POLL_TIMEOUT
   ci_output=$(curl --header "PRIVATE-TOKEN: $GITLAB_PROJECT_TOKEN" --silent "https://${GITLAB_HOSTNAME}/api/v4/projects/${GITLAB_PROJECT_ID}/pipelines/${pipeline_id}")
...
ci_jobs=$(curl --header "PRIVATE-TOKEN: $GITLAB_PROJECT_TOKEN" --silent "https://${GITLAB_HOSTNAME}/api/v4/projects/${GITLAB_PROJECT_ID}/pipelines/${pipeline_id}/jobs" | jq -r '.[] | { id, name, stage }')
...
for JOB_ID in $(echo $ci_jobs | jq -r .id); do
  ...
  curl --header "PRIVATE-TOKEN: $GITLAB_PROJECT_TOKEN" --silent "https://${GITLAB_HOSTNAME}/api/v4/projects/${GITLAB_PROJECT_ID}/jobs/${JOB_ID}/trace"
  echo "##[endgroup]"
done
